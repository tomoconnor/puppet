class apache::redhat inherits apache::base {
  ...
  File["logrotate configuration"] { 
    ...
    content => template("tomoconnor-apache/logrotate-httpd.erb"),
  }

  File["default status module configuration"] {
    path => "${apache::params::conf}/conf.d/status.conf",
    source => "puppet:///modules/tomoconnor-apache/etc/httpd/conf/status.conf",
  }
  ...
  file { ["/usr/local/sbin/a2ensite", "/usr/local/sbin/a2dissite", "/usr/local/sbin/a2enmod", "/usr/local/sbin/a2dismod"]:
    ...
    owner => "root",
    group => "root",
    source => "puppet:///modules/tomoconnor-apache/usr/local/sbin/a2X.redhat",
  }
  ...
  file { "${apache::params::conf}/conf/httpd.conf":
    ...
    content => template("tomoconnor-apache/httpd.conf.erb"),
    seltype => "httpd_config_t",
    notify  => Service["apache"],
    require => Package["apache"],
  }
  ...
  file { "${apache::params::conf}/mods-available":
    ...
    source => $lsbmajdistrelease ? {
      5 => "puppet:///modules/tomoconnor-apache/etc/httpd/mods-available/redhat5/",
      6 => "puppet:///modules/tomoconnor-apache/etc/httpd/mods-available/redhat6/",
    },
    recurse => true,
    mode => 644,
    owner => "root",
