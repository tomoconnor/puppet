...
define apache::vhost-ssl (
  ...
  apache::vhost {$name:
    ...
    config_content => $config_content ? {
      false => $sslonly ? {
        true => template("tomoconnor-apache/vhost-ssl.erb"),
        default => template("tomoconnor-apache/vhost.erb", "tomoconnor-apache/vhost-ssl.erb"),
      },
  ...
  if $ensure == "present" {
    ...
    file { "${apache::params::root}/${name}/ssl/ssleay.cnf":
      ...
      owner   => "root",
      mode    => 0640,
      content => template("tomoconnor-apache/ssleay.cnf.erb"),
